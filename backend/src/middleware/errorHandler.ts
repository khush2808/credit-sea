...
export const globalErrorHandler = (
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    if (error.name === 'ValidationError') {
      ...
      return sendValidationError(res, validationError.errors, validationError.message);
    }
    ...
    if (error.code === 11000) {
      ...
      error = new OperationalError(duplicateError.message, duplicateError.statusCode);
    }
    ...
    if (error.name === 'TokenExpiredError') {
      ...
      error = new OperationalError(jwtExpiredError.message, jwtExpiredError.statusCode);
    }
...
export const notFound = (req: Request, res: Response, next: NextFunction) => {
  const err = new OperationalError(`Can't find ${req.originalUrl} on this server!`, 404);
  next(err);
};
